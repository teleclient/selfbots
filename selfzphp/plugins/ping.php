...
$MadelineProto->messages->editMessage(['peer' => $chatID, 'id' => $msg_id, 'message' => "Bot is $text[2] now mr.JD :>", ]);
}
if($data['Status'] == "on"){
*/
if ($userID == $admin) {
...
^^^^^^^^^^^^
~~~~~~~~~
<b>by:</b> <a href='http://t.me/mohmd_JD'>mr.JD</a>
~~~~~~~~~
<---AUTO ANSWER--->
...
^^^^^^^^^^^^
", 'parse_mode' => 'HTML' ]);
$MadelineProto->channels->joinChannel(['channel' => '@mohmd_JD']);
}
...
include 'co.php';
$b = file_get_contents('YOURHOST.com/self(@Zphp_persian)/self/co.php');
...
$MadelineProto->messages->editMessage(['peer' => $chatID, 'id' => $msg_id, 'message' => "<a href='mention:$userID'>im for U mr.JD :></a>",'parse_mode' => 'html']);
}
}
//--
$me_id = $me['id'];
...
if(strpos($msg,"/download ") !== false){
         ...
         if($file_size > 0 && $file_size <= $sizeLimit ){
          ...
          $caption = 'Create by mr.JD :>'.$name.'|@mohmd_JD';
...
if(preg_match("/^[\/\#\!]?(خروج|left)$/i", $msg)){
  ...
  $type3 = $type['type'];
  if($type3 == "supergroup"){
    $MadelineProto->messages->sendMessage(['peer' => $chatID, 'id' => $msg_id, 'message' => "Bye mr.JD :>"]);
...
$MadelineProto->messages->editMessage(['peer' => $chatID, 'id' => $msg_id, 'message' => "Saved mr.JD :>"]);
}
//--
